      ...
      <table class="tabela">
        ...
        <tbody>
          <tr v-for="g in gabaritosFiltrados" :key="g.id">
            ...
            <td>{{ g.descricao }}</td>
            <td>
              <div v-if="g.anexos?.length"></div>
            </td>
            <td>
              <div v-if="g.anexos?.length">
                <a v-for="anexo in g.anexos" :key="anexo.id" :href="caminhoFoto + anexo.caminho" target="_blank" style="color: var(--cor-primaria)"> {{ anexo.nome_original }} </a>
              </div>
            </td>
            ...
            <td>
              <v-menu>
                <template v-slot:activator="{ props }">
                  <v-btn icon="mdi-dots-horizontal" class="acao-secundaria" v-bind="props" style="width: 2rem; height: 2rem; border: 1px solid var(--cor-separador)"> </v-btn>
                </template>
                <v-list>
      ...
      <fieldset class="grid-2 margem">
        ...
        <div>
          ...
          <ul v-if="gabaritoAtual.anexos.length">
            <li v-for="(file, index) in gabaritoAtual.anexos" :key="index">{{ file.name ?? file.nome_original }} <i class="bi-x-circle" style="color: red" @click.prevent="removerAnexo(index)"></i></li>
          </ul>
        </div>
        <div class="col-2">
  ...
  computed: {
    gabaritosFiltrados() {
      ...
      return this.gabaritos.filter((g) => {
        return (g.codigo && g.codigo.toString().toLowerCase().includes(termo)) || (g.nome && g.nome.toLowerCase().includes(termo)) || (g.descricao && g.descricao.toLowerCase().includes(termo));
      });
    },
  },
