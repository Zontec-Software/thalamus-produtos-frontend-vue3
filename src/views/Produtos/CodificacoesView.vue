            ...
            <tr v-for="item in verbosFiltrados" :key="item.id">
              ...
              <td style="display: flex; justify-content: center">
                <div style="display: flex">
                  <div>
                    <a style="transform: scale(0.8)" @click="editarVerbo(item)" title="Clique para editar o verbo" class="icone-editar"></a>
                  </div>
                  <div>
    ...
    <div class="modal-mask" v-if="showDeleteModal" @click="fecharModalFora">
      <div class="modal-container" style="height: min-content; width: 50rem">
        ...
        <div class="modal-body">
          <span
            >Confirma a exclusão de: <strong>{{ nomeToDelete }}</strong
            >?</span
          >
          <div class="modal-footer">
            <br /><br />
            <button type="button" @click="confirmarExclusao">Confirmar</button> &nbsp;&nbsp; <button type="button" @click="fecharModal" class="acao-secundaria">Cancelar</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal-mask" v-if="showAddModal" @click="fecharModalFora">
      <div class="modal-container" style="height: min-content; width: 50rem">
        <div class="modal-body">
          ...
          <div class="modal-footer">
            <br />
            <button type="button" @click="salvarVerbo">Salvar</button> &nbsp;&nbsp; <button type="button" @click="fecharModal" class="acao-secundaria">Cancelar</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal-mask" v-if="showEditModal" @click="fecharModalFora">
      <div class="modal-container" style="height: min-content; width: 50rem">
        <div class="modal-body">
          ...
          <div class="modal-footer">
            <br />
            <button type="button" @click="salvarEdicao">Salvar</button> &nbsp;&nbsp; <button type="button" @click="fecharModal" class="acao-secundaria">Cancelar</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    verbosFiltrados() {
      ...
      return this.verbos.filter((v) => String(v.id).includes(q) || (v.nome || "").toLowerCase().includes(q));
    },
  },
  ...
  methods: {
    ...
    async salvarEdicao() {
      ...
      try {
        ...
        if (idx !== -1) this.$set ? this.$set(this.verbos, idx, { ...this.verbos[idx], nome: payload.nome }) : (this.verbos[idx] = { ...this.verbos[idx], nome: payload.nome });
