<template>
    <div class="modal-mask" @click.self="$emit('fechar')">
        <div class="jm margem" style="max-width: 80vw; min-width: 50vw">
                        ...
                        <td>
                            <ul class="listaAnexo">
                                <li v-for="a in o.anexos" :key="a.id"><a :href="storageUrl + a.caminho"
                                        target="_blank">{{ a.nome_original }}</a> <i v-if="!readonly" class="bi-trash-fill"
                                        @click="excluirAnexo(o.id, a.id)" title="Excluir anexo"></i></li>
                            </ul>
            ...
            <div v-else class="alinha-centro">
                <span class="fonte-maior">Insira o código da instrução!</span>
            </div>
        </div>
    </div>
</template>
